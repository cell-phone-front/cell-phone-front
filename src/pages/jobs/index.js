...
export default function JobsPage() {
    ...
    getJobs().then((json) => {
      setData(
        (json.jobs || []).map((r) => ({
          ...
          active: r.active === true ? true : r.active === false ? false : "",
          flag: r.flag ?? "Y",
        })),
      );
      setSelected(new Set());
  ...
  const saveHandle = () => {
    const payload = data.map(({ _rid, ...rest }) => ({
      ...rest,
      active: rest.active === "" ? null : rest.active,
    }));

    postJobs(payload).then((ok) => {
  ...
  return (
    <DashboardShell crumbTop="결과분석" crumbCurrent="jobs">
      <div className="">
        <div className="">
          {/* (Tasks 스타일) 헤더: 제목/설명만 */}
          <div className="flex items-center justify-between gap-4 px-3 py-3">
            <div className="flex justify-between gap-4 items-center">
              <h2 className="text-2xl font-bold tracking-tight">Job</h2>
          ...
          {/* 상단 바: 왼쪽(총/선택/삭제) + 오른쪽(행추가/업로드/저장) */}
                ...
                <table className="w-full border-separate border-spacing-0">
                  <thead className="sticky top-0 z-10 bg-slate-200">
                    <tr className="text-left text-sm">
                      ...
                      <th className="min-w-[180px] border-b px-3 py-3 font-medium">
                        작업
                      </th>
                      <th className="min-w-[180px] border-b px-3 py-3 font-medium">
                        기계
                      </th>
                      <th className="min-w-[400px] border-b px-3 py-3 font-medium">
                        설명
                      </th>
                      <th className="min-w-[100px] border-b px-3 py-3 font-medium">
                        활동
                      </th>
                      <th className="min-w-[100px] border-b px-3 py-3 font-semibmediumold">
                        상태
                      </th>
                    </tr>
                  </thead>
                          ...
                          <td className="border-b px-3 py-2">
                            <div className="flex items-center gap-4">
                              ...
                              <select
                                ...
                                className={[
                                  "h-8 rounded-md border bg-white px-3 text-xs",
