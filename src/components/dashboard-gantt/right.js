...
export default function RightBars({ rows = [], rowH = 13 }) {
  const max = useMemo(
    () => Math.max(1, ...rows.map((x) => Number(x.value || 0))),
    [rows],
  );

  // ✅ rowH=13 대응: 바/숫자 높이를 강제로 맞춤
  const barH = Math.max(3, Math.min(6, rowH - 7)); // 13이면 6
  const pillH = Math.max(10, Math.min(14, rowH)); // 13이면 13
