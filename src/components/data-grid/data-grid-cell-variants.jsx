...
export function CheckboxCell({
  ...
  const onCheckboxClick = React.useCallback((event) => {
    event.stopPropagation();
  }, []);

  const onCheckboxMouseDown = React.useCallback((event) => {
    event.stopPropagation();
  }, []);

  const onCheckboxDoubleClick = React.useCallback((event) => {
    event.stopPropagation();
  }, []);
...
export function MultiSelectCell({
  ...
  const [selectedValues, setSelectedValues] = React.useState(cellValue);
...
export function FileCell({
  ...
  const addFiles = React.useCallback(
    async (newFiles, skipUpload = false) => {
      ...
      if (filesToValidate.length > 0) {
        if (!skipUpload) {
          ...
          const newFilesData = filesToValidate.map((f) => ({
            id: crypto.randomUUID(),
            name: f.name,
            size: f.size,
            type: f.type,
  ...
  const onCellDragEnter = React.useCallback((event) => {
    event.preventDefault();
    event.stopPropagation();
  ...
  const onCellDragLeave = React.useCallback((event) => {
    event.preventDefault();
    event.stopPropagation();
  ...
  const onCellDragOver = React.useCallback((event) => {
    event.preventDefault();
    event.stopPropagation();
  }, []);
  ...
  const onDropzoneDragEnter = React.useCallback((event) => {
    event.preventDefault();
    event.stopPropagation();
    setIsDragging(true);
  }, []);

  const onDropzoneDragLeave = React.useCallback((event) => {
    event.preventDefault();
    event.stopPropagation();
    const rect = event.currentTarget.getBoundingClientRect();
    const x = event.clientX;
  ...
  const onDropzoneDragOver = React.useCallback((event) => {
    event.preventDefault();
    event.stopPropagation();
  }, []);
